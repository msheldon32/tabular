...
pub fn evaluate_function<E: ExprEvaluator>(
    ...
    match name {
        ...
        "SUM" => {
            ...
            Ok(CalcType::Float(vals.iter().sum()))
        }
        "AVG" | "AVERAGE" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.is_empty() { Ok(CalcType::Float(0.0)) } else { Ok(CalcType::Float(vals.iter().sum::<f64>() / vals.len() as f64)) }
        }
        "MIN" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            Ok(CalcType::Float(vals.iter().cloned().fold(f64::INFINITY, f64::min)))
        }
        "MAX" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            Ok(CalcType::Float(vals.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
        }
        "COUNT" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            Ok(CalcType::Float(vals.len() as f64))
        }
        "PRODUCT" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            Ok(CalcType::Float(vals.iter().product()))
        }
        "MEDIAN" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.is_empty() { return Ok(CalcType::Float(f64::NAN)); }
            let mut sorted = vals;
            sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            let mid = sorted.len() / 2;
            if sorted.len() % 2 == 0 {
                Ok(CalcType::Float((sorted[mid - 1] + sorted[mid]) / 2.0))
            } else {
                Ok(CalcType::Float(sorted[mid]))
            }
        }
        "MODE" => {
            require_args(name, args, 1)?;
            ...
            Ok(CalcType::Float(mode_key as f64 / 1e10))
        }
        "STDEV" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.len() < 2 { return Ok(CalcType::Float(f64::NAN)); }
            let mean = vals.iter().sum::<f64>() / vals.len() as f64;
            let variance = vals.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (vals.len() - 1) as f64;
            Ok(CalcType::Float(variance.sqrt()))
        }
        "STDEVP" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
        ...
        "VAR" => {
            ...
            if vals.len() < 2 { return Ok(CalcType::Float(f64::NAN)); }
            let mean = vals.iter().sum::<f64>() / vals.len() as f64;
            Ok(CalcType::Float(vals.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (vals.len() - 1) as f64))
        }
        "VARP" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.is_empty() { return Ok(CalcType::Float(f64::NAN)); }
            let mean = vals.iter().sum::<f64>() / vals.len() as f64;
            Ok(CalcType::Float(vals.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / vals.len() as f64))
        }
        "GEOMEAN" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            ...
            Ok(CalcType::Float(product.powf(1.0 / vals.len() as f64)))
        }
        "HARMEAN" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            ...
            Ok(CalcType::Float(vals.len() as f64 / sum_recip))
        }
        "SUMSQ" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            Ok(CalcType::Float(vals.iter().map(|x| x * x).sum()))
        }
        "AVEDEV" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.is_empty() { return Ok(CalcType::Float(f64::NAN); }
            let mean = vals.iter().sum::<f64>() / vals.len() as f64;
            Ok(CalcType::Float(vals.iter().map(|x| (x - mean).abs()).sum::<f64>() / vals.len() as f64))
        }
        "DEVSQ" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            if vals.is_empty() { return Ok(CalcType::Float(0.0)); }
            let mean = vals.iter().sum::<f64>() / vals.len() as f64;
            Ok(CalcType::Float(vals.iter().map(|x| (x - mean).powi(2)).sum()))
        }
        "KURT" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            let n = vals.len() as f64;
            if n < 4.0 { return Ok(CalcType::Float(f64::NAN)); }
            let mean = vals.iter().sum::<f64>() / n;
            let m2 = vals.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / n;
            let m4 = vals.iter().map(|x| (x - mean).powi(4)).sum::<f64>() / n;
            let g2 = m4 / (m2 * m2) - 3.0;
            Ok(CalcType::Float(((n + 1.0) * g2 + 6.0) * (n - 1.0) / ((n - 2.0) * (n - 3.0))))
        }
        "SKEW" => {
            require_args(name, args, 1)?;
            let vals = evaluator.expand(&args[0], results)?;
            let n = vals.len() as f64;
            if n < 3.0 { return Ok(CalcType::Float(f64::NAN)); }
            let mean = vals.iter().sum::<f64>() / n;
            let m2 = vals.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / n;
            let m3 = vals.iter().map(|x| (x - mean).powi(3)).sum::<f64>() / n;
            let g1 = m3 / m2.powf(1.5);
            Ok(CalcType::Float((n * (n - 1.0)).sqrt() / (n - 2.0) * g1))
        }
        ...
        "CORREL" => {
            ...
            if vals1.len() != vals2.len() || vals1.is_empty() { return Ok(CalcType::Float(f64::NAN)); }
            ...
            if std1 == 0.0 || std2 == 0.0 { Ok(CalcType::Float(f64::NAN)) } else { Ok(CalcType::Float(cov / (std1 * std2))) }
        }
        "COVAR" => {
            require_args(name, args, 2)?;
            let vals1 = evaluator.expand(&args[0], results)?;
            let vals2 = evaluator.expand(&args[1], results)?;
            if vals1.len() != vals2.len() || vals1.is_empty() { return CalcType::Float(Ok(f64::NAN)); }
            let n = vals1.len() as f64;
            let mean1 = vals1.iter().sum::<f64>() / n;
            let mean2 = vals2.iter().sum::<f64>() / n;
            Ok(CalcType::Float(vals1.iter().zip(vals2.iter())
                .map(|(a, b)| (a - mean1) * (b - mean2))
                .sum::<f64>() / n))
        }
        "PERCENTILE" => {
            ...
            if vals.is_empty() || k < 0.0 || k > 1.0 { return Ok(CalcType::Float(f64::NAN)); }
            let mut sorted = vals;
            sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            ...
            if lower == upper {
                Ok(CalcType::Float(sorted[lower]))
            } else {
                Ok(CalcType::Float(sorted[lower] + (sorted[upper] - sorted[lower]) * (idx - lower as f64)))
            }
        }
        "QUARTILE" => {
            require_args(name, args, 2)?;
        ...
        "ROUND" => {
            ...
            Ok(CalcType::Float((x * factor).round() / factor))
        }
        "COMBIN" => {
            require_args(name, args, 2)?;
            let n = evaluator.eval(&args[0], results)? as u64;
            ...
            let k = k.min(n - k);
            Ok(CalcType::Float((0..k).fold(1u64, |acc, i| acc * (n - i) / (i + 1)) as f64))
        }
        "PERMUT" => {
            require_args(name, args, 2)?;
            let n = evaluator.eval(&args[0], results)? as u64;
            let k = evaluator.eval(&args[1], results)? as u64;
            if k > n { return Ok(0.0); }
            Ok(CalcType::Float(((n - k + 1)..=n).product::<u64>() as f64))
        }
        "GCD" => {
            require_args(name, args, 2)?;
        ...
        "LCM" => {
            ...
            Ok(CalcType::Float((a / x * b) as f64))
        }

        // === Constants ===
        "PI" => {
            require_args(name, args, 0)?;
            Ok(CalcType::Float(std::f64::consts::PI))
        }
        "E" => {
            require_args(name, args, 0)?;
            Ok(CalcType::Float(std::f64::consts::E))
        }
        "RAND" => {
            require_args(name, args, 0)?;
            Ok(CalcType::Float(rand::thread_rng().gen()))
        }
